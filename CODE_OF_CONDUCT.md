...
reproduced below at v1.3. The canonical text is the one linked above; if the two
ever differ, that one wins.

**Reporting for this project.** kube-crisp is not a CNCF project, so the CNCF and
Kubernetes committees named in the Reporting section below have no jurisdiction
over it. Report an incident here to the maintainer at <manuel@rueg.eu>. The rest
of the standard applies as written.
