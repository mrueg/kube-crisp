...
projection left behind, counting a group as served when a projection declares it or a kube-crisp
`APIService` still answers for it, which is how one loaded from `--projection-dir` shows up —
`--apiservices` finds the registrations one left behind instead — and
