...
The image published from this repository registers three: `aws-rds-iam`, below, and two that need
no SDK because they mint nothing — `token-file` reads a credential something else already refreshes,
and `token-command` runs a command the operator installed and reads what it prints. The second of
those runs nothing at all until an operator enables it. All three work without a rebuild.
