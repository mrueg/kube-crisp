...
type CrispServerOptions struct {
	...
	// CredentialCommandDir holds the commands the token-command credential
	// provider may run, and enables it by existing.
	//
	// Empty by default, which leaves that provider refusing every projection
	// that names it. Running a command inside the API server pod is the
	// server's own privilege — its ServiceAccount, its Secrets, its place in
	// the cluster — and a projection is a cluster object, so whoever may write
	// one would otherwise have all of it. Naming a directory rather than
	// flipping a boolean is what keeps the choice of what runs with the
	// operator: a projection names one file in it, by its bare name, with no
	// arguments.
	CredentialCommandDir string
...
func NewCommandStartCrispServer(ctx context.Context, defaults *CrispServerOptions) *cobra.Command {
	...
	flags.StringVar(&o.CredentialCommandDir, "credential-command-dir", o.CredentialCommandDir,
		"Directory of executables the "+tokencommand.ProviderName+" credential provider may run, one of which a projection names by its bare name and with no arguments. Empty, the default, leaves that provider refusing every projection that names it: a command runs inside this pod with this server's identity, and a projection is a cluster object, so which commands exist is the operator's decision. Point this at a directory somebody else fills and that decision is theirs instead.")
...
func (o *CrispServerOptions) Complete() error {
	...
	if err := tokencommand.Enable(o.CredentialCommandDir); err != nil {
		return fmt.Errorf("--credential-command-dir: %w", err)
	}
