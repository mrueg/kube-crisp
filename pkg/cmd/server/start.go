...
func NewCrispServerOptions(out, errOut io.Writer) *CrispServerOptions {
	...
	//
	// The projection webhook's path is not here, and used to be. The
	// kube-apiserver calls a webhook with whatever credentials the cluster's
	// AdmissionConfiguration gives it for that webhook, and with none at all
	// otherwise — so this path was allowed to everyone, on the grounds that
	// every webhook server in the ecosystem does the same. What that left
	// reachable without any identity was an endpoint that takes a Secret name
	// and a statement, prepares the statement against the database behind the
	// Secret, and says whether the database accepted it: the catalogue of
	// every opted-in database, readable one statement at a time by anything
	// that could reach the Service. It now takes the credentials every other
	// path does; --projection-webhook-allow-anonymous puts it back.
	...
	o.CredentialTokenFileDirs = []string{tokenfile.DefaultDirectory}
...
func NewCommandStartCrispServer(ctx context.Context, defaults *CrispServerOptions) *cobra.Command {
	...
	flags.BoolVar(&o.ProjectionWebhook.AllowAnonymous, "projection-webhook-allow-anonymous", o.ProjectionWebhook.AllowAnonymous,
		"Answer the projection webhook for callers that present no credentials, and without checking that the caller may write customresourceprojections. By default the kube-apiserver has to be given credentials for this webhook through its AdmissionConfiguration, and is refused otherwise. Unsafe: a check prepares the statements in the request against the database behind the Secret it names, so with this set anything that can reach the Service can learn which tables and columns every opted-in database has, and which Secrets exist, with no Kubernetes identity at all.")
	...
	flags.StringSliceVar(&o.CredentialTokenFileDirs, "credential-token-file-dirs", o.CredentialTokenFileDirs,
		"Directories the "+tokenfile.ProviderName+" credential provider may read a credential file from. A projection names the path and a projection is a cluster object, so this is what keeps that from being a way to read any file this process can — the server's own ServiceAccount token included — and hand it to a database as a password. Empty permits none, which leaves the provider registered and refusing every projection that names it.")
...
func (o *CrispServerOptions) Complete() error {
	// The registration and the policy arrive from different places. Which
	// credential providers exist at all is settled by what main links in,
	// before there is a command line to read; which directories the token-file
	// provider may read from is an operator's flag, which does not exist until
	// cobra has parsed one. So the provider is registered there and told here,
	// once, before anything is served.
	if err := tokenfile.Permit(o.CredentialTokenFileDirs); err != nil {
		return fmt.Errorf("--credential-token-file-dirs: %w", err)
	}
	return o.ServerRun.Complete()
}
...
// applyWebhookAnonymity puts the webhook path in the always-allowed set when
// --projection-webhook-allow-anonymous asks for it.
//
// Two things have to give way for an anonymous kube-apiserver to be answered:
// the generic authorization filter, which would refuse an anonymous POST to a
// path it has no rule for before the handler ever saw it, and the handler's own
// check, which the option reaches through the webhook configuration. This is
// the first. Authentication itself still runs — the filter is applied to every
// path — so a caller presenting bad credentials is still turned away; one
// presenting none is simply anonymous, and served.
func (o *CrispServerOptions) applyWebhookAnonymity() {
	if !o.ProjectionWebhook.Enabled || !o.ProjectionWebhook.AllowAnonymous || o.RecommendedOptions.Authorization == nil {
		return
	}
	o.RecommendedOptions.Authorization.WithAlwaysAllowPaths(webhook.Path)
	klog.Warning("--projection-webhook-allow-anonymous: the projection admission webhook is served to " +
		"anything that can reach the Service, with no identity checked. Give the kube-apiserver " +
		"credentials for this webhook through its AdmissionConfiguration and drop the flag")
}
